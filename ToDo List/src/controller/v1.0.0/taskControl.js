 ...
 * 
We get that day task list,else we get that Today's Task list  **/
...
exports.taskComment = async (req, res) => {
  ...
  await Task.findById({ _id: taskid })
    ...
    .then((result) => {
      ...
      if (result.tasksendBy != userid && result._id != user.taskDetails._id) {
        res.json({
        ...
        Task.findOneAndUpdate( { _id: taskid },{ $set: { comment: req.body.comment } });
        res.json({
...
exports.mailSend = async (req, res) => {
    ...
    const result =await Task.find({createdAt: { $gte: startDate, $lt: endDate }}).populate("tasksendBy")
