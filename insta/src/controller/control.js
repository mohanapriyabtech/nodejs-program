...
exports.find = async (req, res) => {
  ...
  await User.findByIdAndUpdate(
    { _id: req.params.userid },
    { $set: { postdetail: await Post.find({ postedBy: req.params.userid }) } }
  );
  await User.find(
    { _id: req.params.userid },
    { username: 1, email: 1, followers: 1, following: 1 }
  )
    .populate("postdetail")
...
exports.unlike = async (req, res) => {
  ...
  if (req.params.userid != post1.likedby) {
    ...
    const result = await Post.findOneAndUpdate(
      { _id: req.params.postid },
      { $unset: { likedby: user }, $inc: { likes: -1 } },
      { new: true }
    )
...
exports.follow = async (req, res) => {
  ...
  if (
    req.params.userid == followdetails.followers ||
    req.params.followid == userdetails.following ||
    req.params.userid == req.params.followid
  ) {
...
exports.unfollow = async (req, res) => {
  ...
  if (req.params.userid == req.params.unfollowid) {
    ...
    req.params.userid == followdetails.followers ||
    req.params.unfollowid == userdetails.following
  ) {
    await User.findByIdAndUpdate(
